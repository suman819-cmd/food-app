...
export default function ResetCodeScreen() {
  ...
  const handlePaste = (text: string) => {
    const pasteText = text.replace(/[^0-9]/g, '').slice(0, 6);
    const newCode = [...code];
    
    pasteText.split('').forEach((char, index) => {
      if (index < 6) {
        newCode[index] = char;
      }
    });
    ...
    // Focus last input
    const lastIndex = Math.min(pasteText.length, 5);
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      const result = await authService.verifyResetCode(email, codeString);
        ...
        <View style={styles.content}>
            ...
            {code.map((digit, index) => (
              <TextInput
                ...
                style={[
                  ...
                  errors.general && styles.codeInputError,
          ...
          <View style={styles.backContainer}>
            <TouchableOpacity 
              ...
              disabled={loading}
            >
              <Text style={[styles.backLink, loading && styles.disabledLink]}>
                <Ionicons name="chevron-back" size={16} /> Use different email
              </Text>
            </TouchableOpacity>
          </View>
        </View>
