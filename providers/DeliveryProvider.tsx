...
import { OrderTracking, OrderStatus, getStatusDescription } from '@/types/delivery';
...
export const DeliveryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const deliveryData: OrderTracking[] = orders.map(order => {
      ...
      if (['preparing', 'ready', 'picked_up', 'on_the_way', 'arrived'].includes(trackingStatus)) {
...
const generateTimeline = (status: OrderStatus, createdAt: string) => {
  ...
  const timeline = [
    { status: 'confirmed' as OrderStatus, timestamp: new Date(baseTime.getTime() + 2 * 60000).toISOString(), description: 'Order confirmed by restaurant' },
    { status: 'preparing' as OrderStatus, timestamp: new Date(baseTime.getTime() + 5 * 60000).toISOString(), description: 'Restaurant started preparing your food' },
  ];

  if (['ready', 'picked_up', 'on_the_way', 'arrived', 'delivered'].includes(status)) {
    timeline.push({ status: 'ready' as OrderStatus, timestamp: new Date(baseTime.getTime() + 15 * 60000).toISOString(), description: 'Food is ready for pickup' });
  }

  if (['picked_up', 'on_the_way', 'arrived', 'delivered'].includes(status)) {
    timeline.push({ status: 'picked_up' as OrderStatus, timestamp: new Date(baseTime.getTime() + 20 * 60000).toISOString(), description: 'Delivery partner picked up your order' });
  }

  if (['on_the_way', 'arrived', 'delivered'].includes(status)) {
    timeline.push({ status: 'on_the_way' as OrderStatus, timestamp: new Date(baseTime.getTime() + 25 * 60000).toISOString(), description: 'Your food is on the way to you' });
  }

  if (['arrived', 'delivered'].includes(status)) {
    timeline.push({ status: 'arrived' as OrderStatus, timestamp: new Date(baseTime.getTime() + 35 * 60000).toISOString(), description: 'Delivery partner has arrived at your location' });
  }

  if (status === 'delivered') {
    timeline.push({ status: 'delivered' as OrderStatus, timestamp: new Date(baseTime.getTime() + 40 * 60000).toISOString(), description: 'Order delivered successfully' });
  }

  return timeline.filter(item => {
    const statusOrder: OrderStatus[] = ['confirmed', 'preparing', 'ready', 'picked_up', 'on_the_way', 'arrived', 'delivered'];
